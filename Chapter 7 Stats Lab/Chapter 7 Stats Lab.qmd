...
Do tree heights in this data set appear to be normally distributed?
:::
...
data.frame(n = c(30,60,150),
  ...
  gt::gt(caption = "Mean and Standard Deviation for different sample sizes n")
```
...
Suppose you were asked the following: "If you measure the height of 60 trees managed by the City of Pittsburgh, what is the probability that the mean height will be less than 25 feet?"
...
CLT_Sum_Ben_30 <- replicate(n = times,
          mean(sample(x = tree_benefits$benefits, 
                      size = 30, 
                      replace = TRUE))
)
